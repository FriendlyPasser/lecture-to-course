...
### Support terminology in context

Glossary entries use the English term as the main label, a conventional Simplified Chinese equivalent, and a short definition consistent with the lecture. Help the learner use important terms rather than merely translate them: add a plain-language explanation, an example from this course, or a contrast with a genuinely confusable term where it clarifies the concept. Use the optional [glossary fields](authoring.md#glossary-as-a-learning-aid) for these additions; there is no quota or need to force every field onto every entry. Include specialized abbreviations and symbols when helpful. Avoid confident Chinese translations when terminology is ambiguous; retain alternatives with context.

Make the example answer a concrete course question, preserving its counts, notation and conditions. For the survey, conditional P(E | S) = 24/40 asks about economics within statistics students; joint P(E ∩ S) = 24/100 asks about both subjects in the whole survey; marginal P(E) = 50/100 counts economics regardless of statistics. The same overlap appears in the first two numerators, but their reference groups differ. These distinctions must agree with the main explanation, equations and quiz feedback. Do not invent an adjacent concept just to fill a comparison: for example, compare an unseen n-gram with an out-of-vocabulary word only in a language-modeling course that actually needs that distinction, explaining that a new combination of known words need not contain an unknown word.

Retain the English original at the first substantive introduction of a key term in each lesson, including Chinese mode, so the student can recognize it in the source material. Author a label such as “条件概率（conditional probability）” there; later occurrences can use the natural Chinese equivalent. This is an authoring decision, not automatic annotation of every occurrence. Translate all added glossary explanations, examples and distinctions for bilingual courses, while keeping English headwords and conventional Chinese equivalents visible in both modes. Record the example and comparison checks in the coverage map.
