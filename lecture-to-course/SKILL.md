...
When a controlled change helps explain a relationship, use a **predict → operate → explain** diagram with one learning goal, a meaningful control, explicit fixed quantities and a visible worked explanation. Prefer the supported offline component in [authoring.md](references/authoring.md#predict--operate--explain-diagrams); keep source data distinct from hypothetical arrangements. Prediction and reflection never gate reading or receive a mastery score. Use a static visual when interaction adds no explanatory value.
...
For interactive diagrams, verify baseline and boundary calculations, conservation of fixed quantities, keyboard operation, reset, and language switching with a changed control and written notes. Confirm essential reasoning remains visible without JavaScript.
