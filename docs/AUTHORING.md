...
Check source pages, formulas and exercise reasoning, then inspect reading and interactions at 1024 and 1440 pixels. Verify source links, typed answers and tolerances, solution reveal/reset, glossary search and keyboard operation. Check that typed responses and exercise state survive a language switch. Read the glossary against the lesson: verify its examples and contrasts, search the added explanation text, and check that bilingual switching translates the prose while preserving both headwords. Confirm the first key-term introduction retains its English original in Chinese mode.
