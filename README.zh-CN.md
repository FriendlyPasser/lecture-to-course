...
| 你遇到的问题 | 课程提供的帮助 |
|---|---|
| “这个公式为什么成立？” | 先建立直觉和简单例子，再逐步连接到公式。 |
| “例题看懂了，换一道还会吗？” | 用新情境检查理解，解释正确答案和容易选错的理由。 |
...
概率论示例先展示 **100 名学生 → 40 名统计课学生 → 其中 24 名也学经济学**。读者先理解“分母里应该是谁”，再接触条件概率公式，最后通过图书馆情境检查自己是否能运用同一个概念。

技能采用以下原则：

1. **先梳理概念与前置知识。** 保留 lecture 边界和来源覆盖，在同一 lecture 内按理解需要合并或重排页面。
2. **先建立理解，再引入符号。** 用问题、图解和小例子连接直觉与正式表达，核心推导和必要条件默认可见。
3. **练习后能追溯来源。** 让学生解释概念、检查适用条件、完成变式；保留来源链接，明确标注自编例子和额外背景。
