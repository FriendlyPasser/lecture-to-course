...
概率论示例可先检查分数与交集，也可直接跳到 **100 名学生 → 40 名统计课学生 → 其中 24 名也学经济学**。同一份调查逐步提出新问题：怎样把群体限制写成公式？知道学生选了某门课，会不会改变另一门课的概率？每个新概念回答前一步留下的问题，再用图书馆情境检查能否迁移这些推理。
...
4. **审核教学并收集反馈。** 为每个核心目标定位讲解、例题和检验题；用本地试学记录收集第一次看不懂的句子、独立推理、新题与间隔后的回访结果。分别报告技术检查、内容审核和实际学习证据。
...
详细标准与已有验证见[教学要求](lecture-to-course/references/teaching.md)和[验证记录](VALIDATION.md)。阅读进度只表示读到了哪里，不代表已经掌握。

[示例教学审核](demo/teaching-review.md)记录了目标覆盖及内容缺口；[学生试学方案](demo/learner-trial.md)提供新题和主持人参考要点。目前尚未开展真实学生试学，因此不能据此宣称学习效果提高。技能包内附有可复用的[验收与反馈模板](lecture-to-course/references/teaching-review.md)。
